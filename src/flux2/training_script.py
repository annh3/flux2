"""
Full fine-tuning script for Klein9B (FLUX.2) using a HuggingFace dataset.
...
#     --save_every 9999 --val_size 2 --val_num_steps 5 --output_dir ft_output
...
#### TensorBoard Launch ####
# local:
#   tensorboard --logdir ft_output/runs
#   open http://localhost:6006

# remote:
#   (remote) tensorboard --logdir ~/flux2/src/flux2/ft_output/runs --host 0.0.0.0 --port 6006
...
def encode_batch(
    ...
    txt_tokens, txt_ids = batched_prc_txt(txt_embeds)          # [B, T, D], [B, T, 4]
...
def train(args):
    ...
    print(f"Trainable: {n_trainable:,} parameters")
    ...
    while step < args.steps:
        for batch in loader:
            ...
            if step % args.save_every == 0:
                ckpt = os.path.join(args.output_dir, f"checkpoint_step{step:06d}.pt")
                torch.save(model.state_dict(), ckpt)
                print(f"Saved: {ckpt}")

    final = os.path.join(args.output_dir, "checkpoint_final.pt")
    torch.save(model.state_dict(), final)
    print(f"Done. Checkpoint saved to {final}")
...
def parse_args():
    ...
    p.add_argument("--output_dir",              default="ft_output")
    ...
    p.add_argument("--gradient_checkpointing",  action="store_true",
                   help="enable gradient checkpointing on single stream blocks to reduce VRAM usage")
    # Validation
